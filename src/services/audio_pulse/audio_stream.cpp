...
namespace JappeStudios::JappeOS::JappeOSCore::Services::AudioPulse
{
    ...
    AudioStream::AudioStream(AudioService& source,
                             ...
                             _volume         (_conn, _iface, "Volume", 0.0, true, [this](const double& val) { OnSetVolume(val); }),
                             _muted          (_conn, _iface, "Muted", false, true, [this](const bool& val) { OnSetMuted(val); })
    {
    }

    void AudioStream::OnSetDevice(const ObjectPath& devicePath) const
    {
    ...
    void AudioStream::OnSetVolume(const double volume) const
    {
        if (_suppressPropertyCallbacks)
            return;
    ...
    void AudioStream::OnSetMuted(const bool muted) const
    {
        if (_suppressPropertyCallbacks)
            return;
